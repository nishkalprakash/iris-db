...
ORIG_DB_BASE_ = BASE_ / Path("SEGMENTATION_GROUND_TRUTHS/IRISSEG-EP/IRISSEG-EP-Masks-r1/IRISSEG-EP-Masks/masks/iitd")
...
new_img_tag_info = {
    ...
    new_img_tag: {
        ...
        "orig_base_path": str(ORIG_DB_BASE_.as_posix()),
    }
}
...
#%%
base_ = DS_BASE_
# img_tags = new_img_tag_info[new_img_tag]
# session_id = 1
db.close()
meta.close()
#%%
...
for img_ in tqdm(images):
    ...
    #             
    pattern = r'Operator[AB]_(\d+)-[AB]_(\d+)'  # Regex pattern to match the filename structure
    match = re.match(pattern, img_.stem)
    if not match:
        lg.warning(f"Filename {img_.name} does not match the expected pattern.")
        continue
    person_id, sample_id_person = map(str,map(int, match.groups()))
    # person_id, sample_id_person = map(int,img_.stem.split('_'))  # 
    # eye = "L" # default left eye
    # if person_id == 0:
        # person_id = 1000
    # eye_id = f"{person_id}_{eye}"
    # image_id = f"{eye_id}_{sample_id}"
    doc = db.find_one({'person_id': person_id,'sample_id_person': sample_id_person},{
        'image_id': 1,
        'eye_id': 1,
        'sample_id': 1,
        'img_tags':1, 
    ...
    new_filename_ = Path(f"{doc['eye_id']}_{doc['sample_id']}{ext}")
    path_ = base_ / new_img_tag/ doc['eye_id'] / new_filename_
    img=Image.open(img_)
    w,h = img.size
    img_specs = {
        'ext': ext,
